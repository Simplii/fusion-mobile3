...
class CallHistory extends FusionModel {
  ...
  isInternal(String domain) {
    ...
    if (direction == 'inbound')
      return to!.substring(to!.length - domain.length).toLowerCase() ==
          domain.toLowerCase();
    else
      return from!.substring(from!.length - domain.length).toLowerCase() ==
          domain.toLowerCase();
  }
...
class CallHistoryStore extends FusionStore<CallHistory> {
  ...
  getPersisted(
      int limit, int offset, Function(List<CallHistory>, bool, bool) callback) {
