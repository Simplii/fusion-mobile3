...
class ContactProfileView extends StatefulWidget {
  ...
  ContactProfileView(
      this._fusionConnection, this._softphone, this._contact, this.refreshUi,
      {Key? key})
      : super(key: key);
...
class _ContactProfileViewState extends State<ContactProfileView> {
              ...
              child: FusionDropdown(
                  ...
                  button: Container(
                      ...
                      padding: EdgeInsets.only(
                          right: 16, left: 16, top: 6, bottom: 4),
    ...
    return [
      _renderFieldGroup(
          ...
          _contact!.phoneNumbers
              ...
              .toList()
              .cast<Widget>()),
      if (_contact != null && _contact!.emails.isNotEmpty)
        _renderFieldGroup(
            "mail_filled_dark",
            _contact!.emails
                .map((email) {
  ...
  List<Widget> _getTimeline() {
    if (_isSpinning()) return [_spinner()];

    if (_isEmpty()) return [_noHistoryMessage()];
