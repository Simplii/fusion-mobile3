...
class _SMSConversationViewState extends State<SMSConversationView> {
  ...
  _header() {
    ...
    if (_conversation!.number.contains("@")) {
      _fusionConnection!.coworkers
          .getRecord(_conversation!.number, (coworker) => _coworker = coworker);
    }
    ...
    Widget groupMessageHeader = Expanded(
        child: Padding(
      ...
      child: LimitedBox(
        ...
        child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            ImageProvider image =
                _conversation!.contacts[index].profileImage != null
    ...
    List<List<String>> contactsToAdd = _conversation.isBroadcast
        ? []
        : List.generate(
            unknowContacts.length,
            (index) => [
                  "Add ${unknowContacts[index].name} as a new contact",
                  "addContact-${index} "
                ],
            growable: false);
    ...
    return Column(children: [
      Center(
          child: Container(
              ...
              height: 5)),
      Row(children: [
        if (_conversation!.isGroup) groupMessageHeader,
        if (!_conversation!.isGroup) ...singleMessageHeader,
              ...
              if (chosen == "contactprofile") {
                ...
                if (_deleteConvo != null && _conversation != null) {
                  ...
                  _fusionConnection!.conversations
                      .deleteConversation(_conversation!, _selectedGroupId);
                }
                Navigator.pop(context, true);
              } else if (chosen.contains("addContact")) {
                int chosenUnknownContactIndex =
                    int.parse(chosen.split('-').last);
                Future.delayed(Duration(milliseconds: 10), () {
                              ...
                              return GestureDetector(
                                ...
                                child: Container(
                                  ...
                                  child: Row(
                                    ...
                                    children: [
                                      CircleAvatar(
                                        foregroundImage: coworkers[index].uid ==
                                                'Unassigned'
                                            ? AssetImage(
                          ...
                          return AlertDialog(
                            ...
                            content: TextFormField(
                              ...
                              decoration: InputDecoration(
                                ...
                                errorText:
                                    showError ? "Unable to rename" : null,
                              ),
                            ),
                            actions: [
                              TextButton(
  ...
  List<List<String>> bottomsheetOptions(
  ...
  String convoLabel() {
    ...
    if (_conversation.groupName.isNotEmpty) {
      ...
      label =
          "${_conversation.contacts[0].firstName} ${_conversation.contacts[0].lastName}";
    }
    return label;
  }
  ...
  _onNumberSelect(String newNumber) {
    ...
    _updateQuickMessages(selectedDept: dept?.id ?? DepartmentIds.Personal);
    setState(() {
  ...
  _openQuickResponses() {
    List<String?> messages = quickResponses
        .map(
          (qr) => qr.message,
        )
        .toList();
                  ...
                  onComplete: (DateTime? selectedDateTime) {
                    setState(() {
                      if (selectedDateTime != null)
                        secheduleIsSet = selectedDateTime;
                    });
                  }),
            ));
  }
                ...
                child: Row(
                  children: [
                    Text(
                      ...
                      style: TextStyle(
                          fontWeight: FontWeight.bold,
                          color: assignedTo.uid!.toLowerCase() !=
                                  _fusionConnection
                                      .getUid()
                                      .toString()
                                      .toLowerCase()
            ...
            Row(children: [
              ...
              Expanded(
                ...
                Container(
                    ...
                    margin:
                        EdgeInsets.only(top: _mediaToSend.length > 0 ? 119 : 0),
                    decoration: BoxDecoration(
                        color: Colors.white,
                        border: Border.all(
                            color: Color.fromARGB(255, 229, 227, 227),
                        ...
                        borderRadius: BorderRadius.only(
                          topLeft:
                              Radius.circular(_mediaToSend.length > 0 ? 0 : 8),
...
class _ConvoMessagesListState extends State<ConvoMessagesList> {
  ...
  Function(SMSConversation, SMSMessage?)? get _deleteMessage =>
      widget._deleteConvo;
  Function? get _setOnMessagePosted => widget.setOnMessagePosted;
  Function(SMSConversation) get _changeConvo => widget.changeConvo;
  ...
  Future<SMSConversation?> _departmentSwitch() async {
    SMSConversation? convo = await _fusionConnection!.messages
        .checkExistingConversation(_selectedGroupId, _conversation!.myNumber,
...
class _SMSMessageViewState extends State<SMSMessageView> {
  ...
  _deleteMessage(SMSMessage? message) {
    ...
    if (_messages.length == 1)
      ...
      widget._deleteMessage!(
          _conversation!, _messages.reversed.elementAt(_messages.length - 2));
  ...
  Widget build(BuildContext context) {
    ...
    bool scheduledMessage = _message!.scheduledAt != null
        ? DateTime.parse(_message!.scheduledAt!)
            .toLocal()
    ...
    if (_message!.from != _conversation!.myNumber) {
      ...
      children.add(ContactCircle.withDiameter(
          matchedContact ?? _conversation!.contacts,
          _conversation!.crmContacts,
          44));
      ...
      if (_message!.user != null) {
        myContact = _fusionConnection!.coworkers
            .lookupCoworker(_message!.user!.split('@')[0] +
                "@" +
        ...
        return await showDialog(
          ...
          builder: (BuildContext context) {
            return AlertDialog(
              ...
              content:
                  const Text("Are you sure you wish to delete this message?"),
              actions: <Widget>[
                TextButton(
                    style: TextButton.styleFrom(
